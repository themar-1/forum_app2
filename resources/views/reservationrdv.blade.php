                ...
                @if ($entreprises->count() > 0)
                    ...
                    <div class="child-container">
                        @foreach ($entreprises as $entreprise)
                            @if ($entreprise->status === 0 || $entreprise->status === 1)
                                    ...
                                    <div class="testimonial-item bg-light rounded p-4">
                                        ...
                                        <div class="d-flex align-items-center">
                                            <img class="img-fluid flex-shrink-0 rounded"
                                                src="{{ asset('img/logos/' . $entreprise->logo) }}"
                                                style="width: 50px; height: 50px;">
                                            <div class="ps-3">
                                                <h5 class="mb-1">{{ $entreprise->raisonabregee }}</h5>
                                                <a
                                                    href="http://%7B%7B/ $entreprise->web }}"><small>{{ $entreprise->web }}</small></a>
                                                <div class="text-center">
                                                    <form method="POST"
                                                        action="{{ route('apply-for-interview', ['entrepriseId' => $entreprise->id]) }}">
                                                        @csrf
